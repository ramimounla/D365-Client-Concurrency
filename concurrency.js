...
packtNs.concurrency.checkLastModified = function (recordLoadedVersion)
{
    ...
    req.open("GET", encodeURI(Xrm.Page.context.getClientUrl() + "/api/data/v8.1/" + _schemaName + "(" + Xrm.Page.data.entity.getId().substring(1, 37) + ")?$select=_modifiedby_value&$expand=modifiedby($select=fullname)"), false);
    req.send();
